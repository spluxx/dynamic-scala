...
import structure.datatype.DirectedGraph
import structure.typeclass.{Edge, Graph, Node, NodeCtx}
...
trait GraphInstance {
  implicit def directedGraph[I, W]: Graph[DirectedGraph, I, W] = new Graph[DirectedGraph, I, W] {
    ...
    override def nodes(graph: DirectedGraph[I, W]): List[Node[I]] = graph.nodes.map(_._2.node).toList
  }
}
